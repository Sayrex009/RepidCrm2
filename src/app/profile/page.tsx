...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (
      ...
      fetch(
        `https://crmm.repid.uz/employee/detail?user_id=${session.user.id}`,
        {
          headers: {
        ...
        .then(async (res) => {
          if (!res.ok) {
            ...
            throw new Error(
              errorData.detail || "Failed to fetch employee data"
            );
          }
          return res.json();
        })
    ...
    <main className="px-[15px] sm:px-[30px] pt-[20px] pb-[40px] lg:overflow-auto min-h-screen bg-gray-200">
      <div>
        <div className="flex items-center mt-24 justify-between mb-5 md:mb-7">
          <h1 className="text-[#26273F] font-bold text-[24px] md:text-[32px]">
            Profil
          </h1>
        </div>
          ...
          <>
              ...
              <div className="relative">
                <div className="relative">
                  <div className="max-w-[80px] min-w-[80px] max-h-[80px] min-h-[80px] md:max-w-[100px] md:min-w-[100px] md:max-h-[100px] md:min-h-[100px] overflow-hidden rounded-full bg-gray-200 flex items-center justify-center border-2 border-gray-300 shadow-md">
                    {avatar &&
                    !["None", "null", "undefined", ""].includes(avatar) ? (
                      <Image
                        src={
            ...
            <div className="w-full p-5 sm:p-[30px] bg-white rounded-lg">
              <form className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-5 gap-y-6">
                <div>
                  <label className="text-[#7D8592] font-medium block mb-2">
                    Ismi
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <div className="ant-select ant-select-lg ant-select-outlined w-full md:min-h-[46px]">
                    <input
                      type="text"
                      value={selectedPosition}
                      onChange={(e) => setSelectedPosition(e.target.value)}
                      className="w-full md:text-[17px] h-[46px] px-3 border border-gray-300 rounded"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="ant-picker ant-picker-large ant-picker-outlined w-full md:h-[46px] px-3 border border-gray-300 rounded"
                  />
                </div>
                ...
                <div className="flex gap-3 justify-between col-span-full mt-4">
                  <button
                    type="button"
                    className="ant-btn ant-btn-primary ant-btn-lg md:h-[50px] md:text-lg px-6 py-2 bg-[#F48C06] text-white rounded"
                  >
                    Parolni o'zgartirish
                  </button>
                  <button
                    type="submit"
                    className="ant-btn ant-btn-primary ant-btn-lg w-[200px] md:h-[50px] md:text-lg px-6 py-2 bg-[#F48C06] text-white rounded"
                  >
                    Saqlash
                  </button>
                </div>
